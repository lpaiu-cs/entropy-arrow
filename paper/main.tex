% Mode-matched record horizons in reversible dynamics — PRE-target draft v0.2
...
% TODO: real affiliation before submission
...
carries it}; the thermalization time bounds that lifetime and is attained by
boundary-scale records. The law fails exactly where it must---a conservation law
protecting the record decouples $\tstar$ from $\tS$---and records are stored redundantly,
...
quantify the sense in which the experienced past is the direction of the entropy
...
$1$--$5\%$. ``Record lifetime $=$ thermalization time'' is the \emph{supremum} of this law,
attained by records written in the slowest mode.
\item \textbf{Universality} (Sec.~\ref{sec:universality}): the boundary-scale
proportionality reproduces in a continuous hard-disk gas and in reversible Clifford
circuits (up to $N=128$ qubits), and survives a Haar-random statevector control.
...
vs $156.3$). The one censored run can only \emph{lengthen} a horizon: its record never
crosses the $\tfrac12$-bit gate within $T=2400$ while its four sibling seeds cross at
$\tstar\approx192$, so it cannot weaken the law's direction; folding it in as the point
value $\tstar=T$ would move the through-origin fit to $3.0$ purely through that single
high-leverage point, which is why we quote the resolved-run fit and disclose the censored
run instead. Re-deriving both crossings from the stored curves over a grid of gate
...
$\lambda=w/m$. As a decoder-free reference for each mode's lifetime we use the
noise-corrected class-mean separation power $\|\Delta(t)\|^2$ (the squared distance between
the two classes' mean coarse states, with the $K$-world sampling floor subtracted), whose
decay time $\tau_{\Delta^2}$ tracks the discriminating signal wherever it drifts; because
...
---errors of $0.5\%,\,4.7\%,\,1.9\%,\,4.0\%$. The decoder-free mode times agree,
...
\max_{\mathrm{records}}\tstar \simeq \mathcal{O}(1)\,\tS\;}
\label{eq:modelaw}
\end{equation}
with the maximum attained by records written in the slowest thermalizing mode---which is
what boundary-condition facts (``the gas started on the left'') naturally are. The
falsification experiment of Sec.~\ref{sec:falsification} is the limiting case of
Eq.~\eqref{eq:modelaw}: a protected mode has $\tau_{\mathrm{mode}}=\infty$.
...
Middle: decoder-free mode power $\|\Delta(t)\|^2$ on a log scale---each wavelength has its
own relaxation time. Right: $\tstar(m)$ against $\tau_{\Delta^2}(m)$ with the three
...
under $t/\tS$. The complementary \emph{mode-mismatch control} behaves as
Sec.~\ref{sec:modes} requires: growing the box while holding the blob fixed decouples the
two spatial scales (filling the box versus erasing a local asymmetry) and the ratio then
drifts---which is the mode-resolved law reasserting itself, not a failure of the
proportionality. (This substrate is float-chaotic, so exact values vary at the percent
level with the floating-point environment; an independent re-run on different hardware
gives $\kap=1.04$ with the same flatness \cite{repo}.)
...
onto single master curves. Holding the blob fixed while growing the box (a deliberate
mode mismatch) makes the ratio drift, as the mode-resolved law requires.}
\label{fig:md}
\end{figure*}
...
rather than a tautology of our definitions.
...
(v) the law fails precisely when a conservation law protects the record---the
$\tau\to\infty$ limit of the same statement; (vi) records are stored redundantly, with an
...
thermalization time itself; objectivity while alive is redundancy; and the only way to
outlive the gradient is a conservation law---which is engineering (memory protection), not
...
The decoder-free mode power is $\|\Delta(t)\|^2=\|\bar V_0(t)-\bar V_1(t)\|^2$ minus its
$K$-world sampling floor, where $\bar V_c$ is the class-mean coarse state.
