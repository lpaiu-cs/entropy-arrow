...
(Fig.~\ref{fig:anomaly}). The diagnosis: the planted marker is a solid patch, a fully
occupied (or fully empty) $2\times2$ block is a \emph{fixed point of every block rule} of
this automaton, and in this one quenched world the marker's erosion stalls. At $t=2400$,
...
box (a void remnant). The realization is non-ergodic exactly where the record lives: its
mode does not relax ($\tau_{\mathrm{mode}}=\infty$), the protected limit of
Eq.~\eqref{eq:modelaw} and of Sec.~\ref{sec:falsification}, arising here spontaneously
from quenched disorder rather than by engineering. We therefore state the horizon law as
holding in $24/25$ realizations, with the exception classified as a frozen-record sector
rather than folded into the fit (as a point value at its lower bound $\tstar\ge T$ it
would shift the through-origin fit to $3.0$ through a single high-leverage point). Re-deriving both crossings from the stored curves over a grid of gate
...
$0.42$, one of five quenched worlds). Left: the anomalous world's record never dies---it
remains perfectly readable when the run is extended to $5\times$ the original window
($\approx48\,\tS$)---while a resolved sibling world's record dies on schedule.
...
point of every rule, so a stalled erosion stores the record indefinitely: the
...
restriction is not pedantry: a non-relaxing mode escapes the bound entirely, whether
engineered (the conservation law of Sec.~\ref{sec:falsification}) or spontaneous (the
frozen realization of Sec.~\ref{sec:horizon})---both are the
$\tau_{\mathrm{mode}}=\infty$ limit of Eq.~\eqref{eq:modelaw}.
...
which a frozen structure---no engineered gate---stores the record indefinitely.
...
occasionally build its own memory protection, and when it does, the record steps outside
the thermodynamic clock. In companion experiments in the same testbed \cite{repo}, the
...
\cite{repo}. Simulation code, analysis, and manuscript drafting were carried out with
substantial assistance from an AI system (Claude, Anthropic); all experiments were
independently re-executed and the reported numbers verified against the committed data,
