...
for $48\,\tS$; the incidence of this frozen sector is itself measured, closed at weak
disorder ($0/448$ worlds) and opening sharply ($5.5\%$, then $43\%$) at the strongest
scrambling rates---and records are stored redundantly,
with effective redundancy growing as $N^{\alpha}$, $\alpha_\infty=0.92\pm0.04$
(conservative) classically and $\alpha=1$ for quantum broadcast states. Finally, the
horizon needs no external analyst: an \emph{endogenous} one-bit observer---a reversible
Maxwell demon whose sensor is fixed at the boundary and never retrained, writing to blank
tape by reversible \textsc{cnot}, erasures counted at the Landauer bound---sees the same
horizon ($\tstar_{\mathrm{demon}}=0.95\,\tS$ on the lattice, $0.54\,\tS$ in the gas),
cannot learn in equilibrium ($0.02$ bit), and its best recall decays with the record's own
mode: durable memory is bought only with blank tape spent before the record has relaxed.
These results quantify the sense in which the recorded past is the direction of the entropy
...
\item \textbf{A falsification boundary, with a measured incidence}
(Secs.~\ref{sec:falsification}, \ref{sec:horizon}): the law is contingent on ergodicity.
A conservation law protecting the record makes $\tstar$ diverge while $\tS$ stays
finite---the extreme case of a non-relaxing mode---and the spontaneous counterpart, the
kinetically frozen record, has a sharp disorder-driven onset: closed at weak disorder
($0/448$ worlds), $5.5\%$ at scatter $0.42$, $43\%$ at $0.50$.
...
\item \textbf{An endogenous observer} (Sec.~\ref{sec:demon}): replacing the retrained
decoder by a physical one-bit demon---sensor calibrated once at the boundary and never
retrained, records written to blank tape by reversible \textsc{cnot}, erasures counted at
the Landauer bound \cite{szilard1929,landauer1961,bennett1982,parrondo2015}---reproduces
the horizon law on the lattice ($\kap=0.95$) and in the gas ($\kap=0.54$), flips with the
boundary, and cannot learn in equilibrium. Its thermodynamic ledger makes the cost of
memory explicit: keeping the fact \emph{currently} known until $\tstar$ costs $\sim9$
erased bits per bit of content, and recall from a tape started at $t_{\mathrm{start}}$
decays with the record's own mode---the low-entropy boundary is a non-renewable resource.
\end{enumerate}
...
statements are lower bounds.

Is the frozen realization a fluke? We measured its \emph{incidence}
(Fig.~\ref{fig:incidence}): $798$ additional independent quenched worlds across six
scrambling rates, each evolved to $T\approx8\,\tS$ and classified by the same operational
readout (frozen: late-time MI $\ge0.8$ bit; partial/slow: $[0.3,0.8)$ bit, reported
separately and never folded in). The frozen sector has a sharp, disorder-driven onset. It
is closed at scatter $0.15$--$0.35$ ($0$ of $448$ worlds; $95\%$ upper bounds
$2.5$--$3.8\%$), opens at $0.42$ ($11/200=5.5\%$, CI $[3.1,9.6]\%$---consistent with the
$1/25$ of the horizon sweep), and reaches $43.3\%$ ($65/150$, CI $[35.7,51.3]\%$) at
scatter $0.50$; the partial/slow population tracks the onset ($0\to3\to29\to57$ worlds),
and at the strongest disorder $81\%$ of worlds retain significant record signal at
$8\,\tS$. The horizon law therefore lives in a \emph{quantified} domain: across the
disorder range where it was measured, the ergodic sector comprises $94.5$--$100\%$ of
realizations, while stronger scrambling---which in this substrate also means slower
transport---carries the gas into a regime where kinetically frozen records are common.
The protected sector of Sec.~\ref{sec:falsification} is not only engineerable; it opens
spontaneously, with a measurable rate, as a function of a single disorder knob.

Re-deriving both crossings from the stored curves over a grid of gate
...
\begin{figure*}
\includegraphics[width=0.92\textwidth]{T7_frozen_incidence.png}
\caption{\textbf{The frozen sector has a measured incidence with a sharp disorder-driven
onset} ($798$ quenched worlds, $\sim100$--$200$ per scrambling rate, each evolved to
$T\approx8\,\tS$). Left: the fraction of worlds whose record is still essentially
perfectly readable (late MI $\ge0.8$ bit) at $T\approx8\,\tS$, with $95\%$ Wilson
intervals and raw counts: $0/448$ at scatter $0.15$--$0.35$, $5.5\%$ at $0.42$, $43.3\%$
at $0.50$. Right: the late-time MI distributions behind the classification---worlds pile
up dead (left peak) or frozen (right peak), with a partial/slow band opening at the onset
($0\to3\to29\to57$ worlds; classification thresholds dotted).}
\label{fig:incidence}
\end{figure*}
...
and substrate-dependent, Eq.~\eqref{eq:kappa}; the law is the flat proportionality. The
last two rows replace the retrained decoder by the fixed endogenous one-bit demon of
Sec.~\ref{sec:demon}---a weaker reader (lower $\kap$), reading the same clock.}
\label{tab:kappa}
\begin{ruledtabular}
\begin{tabular}{llll}
...
\hline
CA, demon readout  & bit-reversible          & $3.5\times$ & $0.95$ \\
Gas, demon readout & Hamiltonian             & $5.1\times$ & $0.54$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\section{An endogenous observer: the horizon without an external analyst}
\label{sec:demon}

Every readout so far---including the fixed-template control of Sec.~\ref{sec:modes}---is
applied by an external analyst with fresh access to the whole coarse state at every sample
time. A physical observer is a subsystem of the same reversible dynamics: its detector is
fixed when the event happens (one cannot re-fit a memory to a fact already forgotten), and
writing a record has a thermodynamic price
\cite{szilard1929,landauer1961,bennett1982,parrondo2015}. We therefore build a reversible
Maxwell demon inside the lattice gas and ask whether the horizon law survives when the
observer is endogenous and its bookkeeping is honest. The demon has a \emph{fixed one-bit
sensor}, calibrated once at the boundary as the class contrast
$w=\bar V_0(0)-\bar V_1(0)$ over the training worlds and never retrained; its reading is
$o(t)=\mathbf{1}[\,w\cdot V(t)>\theta\,]$, and its knowledge of the fact is the held-out
mutual information $I\bigl(o(t);F\bigr)\le1$ bit. Recording is the reversible \textsc{cnot}
$\text{tape}\oplus\!=o(t)$ onto a blank cell---von Neumann premeasurement---so a blank cell
is \emph{required} to record, and resetting a used cell is an erasure, which we count in
units of the Landauer bound (the athermal substrate has no temperature; the ledger counts
erased bits, each costing $k_BT\ln2$ in any isothermal implementation). Note the demon does
not threaten the second law---Szilard, Landauer and Bennett settled that
\cite{szilard1929,landauer1961,bennett1982}; we run the argument in the opposite direction
and ask what the exorcised demon's \emph{knowledge} is slaved to.

Three results (Fig.~\ref{fig:demon}). First, the dumb one-bit physical detector sees the
\emph{same horizon} as the trained multi-cell decoder: across four scrambling rates
($\tS=26$--$90$, $3.5\times$), $\tstar_{\mathrm{demon}}=0.95\,\tS$ with per-rate ratios
$0.68$--$1.05$ (through-origin $R^2=0.73$, resolution-limited by the one-bit reading at the
sample stride)---compare the decoder's $\kap=1.08$ (Table~\ref{tab:kappa}). The horizon is
in the state, not in the sophistication of the readout. Relocating the low-entropy boundary
to the opposite time end flips the demon's faithful readings with it
($I(o;F)$: $0.01$ at $t=0$, $1.00$ at $t=T$). Second, the \emph{equilibrium control}: the
same fact presented to a pre-thermalized gas (the fact survives only in the microstate, not
the macrostate) yields a coin-flip reading---mean $I(o;F)=0.02$ bit, never crossing the
$\tfrac12$-bit threshold over the whole run, against a forward peak of $1.00$. Without a
low-entropy boundary there is nothing for a physical memory to record.

Third, the thermodynamic ledger. A \emph{cyclic} demon (one tape cell, erased and rewritten
each epoch) holds at most one bit forever, yet its erasure bill grows linearly: keeping the
fact \emph{currently} known until $\tstar_{\mathrm{demon}}$ costs $\sim9$ erased bits per
bit of content at the representative rate---the running price of a live memory of a
decaying record. An \emph{accumulate} demon (a fresh blank cell each epoch, old cells
frozen) escapes the recurring bill, but only cells written early carry the fact: the best
recall available from a tape \emph{started} at $t_{\mathrm{start}}$ is $1.00$ bit at
$t_{\mathrm{start}}=0$ for every rate, decays along the record's own mode---the curves of
all four rates collapse on $t_{\mathrm{start}}/\tS$ with the knee at ${\approx}\,\kap$---and
reaches the estimator noise floor ($0.08$--$0.18$ bit) past ${\sim}3\,\tS$. No amount of
blank tape spent after the record has relaxed buys the fact back: durable memory is
purchased with low-entropy resource spent \emph{while the gradient is alive}, the same
resource the boundary condition supplies.

Why does a \emph{fixed} sensor work at all? By the mechanism of
Secs.~\ref{sec:mechanism}--\ref{sec:modes}: in a diffusive substrate the boundary-scale
record rides the slowest relaxation mode, which decays in place (amplitude shrinks, spatial
shape preserved), so a sensor matched to it at the boundary stays matched. This predicts
the construction generalizes exactly as far as mode matching does, and it does: in the
hard-disk gas the identical fixed-contrast sensor inherits the law,
$\tstar_{\mathrm{demon}}=0.54\,\tS$ ($R^2=0.92$) over the full $5.1\times$ self-similar
sweep of Sec.~\ref{sec:universality} (per-size ratios $0.67$--$0.52$: a weaker reader than
the retrained decoder's $\kap=1.00$, reading the same clock). In the ballistically
scrambling Clifford brickwork, by contrast, a fixed local sensor (a \textsc{cnot} copy of a
sensor qubit onto a blank ancilla) loses the record at an $N$-independent $O(1)$ butterfly
time even though the actively decoded window information survives to ${\approx}0.8\,\tS$
(Fig.~\ref{fig:clifford})---the mode-mismatch expectation, since a ballistic scrambler has
no slowly relaxing local mode to ride; a systematic study of that crossover is deferred to
companion work.

\begin{figure*}
\includegraphics[width=\textwidth]{T9_maxwell_demon.png}
\caption{\textbf{The endogenous demon} (lattice gas, $K=40$ worlds per class, three
disorder seeds). Left (representative rate): the fixed one-bit sensor's held-out
$I(o;F)$ falls below $\tfrac12$ bit at the entropy-saturation time (blue); relocating the
low-entropy boundary to the future end flips the readings (orange, dashed); a demon in a
pre-thermalized gas never learns the fact (grey); the cyclic demon's erasure bill grows
linearly throughout (red, right axis). Middle: $\tstar_{\mathrm{demon}}$ vs.\ $\tS$ across
four scrambling rates; the fixed sensor reproduces the horizon law with $\kap=0.95$.
Right: the accumulate demon's best recall from a tape started at $t_{\mathrm{start}}$,
for all four rates, collapsing on $t_{\mathrm{start}}/\tS$: recall decays with the
record's own mode and blank tape spent past ${\sim}3\,\tS$ buys only the noise floor.}
\label{fig:demon}
\end{figure*}
...
Sec.~\ref{sec:falsification}) or spontaneous, and the spontaneous sector has a measured,
sharply-onsetting incidence in disorder strength ($0/448\to5.5\%\to43\%$,
...
the noiseless quantum broadcast; (vii) none of this requires an external analyst: an
endogenous, thermodynamically-billed one-bit demon---sensor fixed at the boundary,
reversible tape---sees the same horizon on the lattice and in the gas, flips with the
boundary, cannot learn in equilibrium, and its recall decays with the record's own mode,
so durable memory is purchased with blank tape spent while the gradient is alive.
...
Several extensions seem most valuable. (i) A quantitative tie of the mode times to
...
low-entropy past---Reichenbach's common cause as a measurable statement. (iv) The measured
onset of the frozen sector invites a mechanism study: what structural change controls it,
how the onset scales with marker geometry and system size, and how this classical sector
relates to the broader physics of weak ergodicity breaking and fragmentation-protected
memory. (v) The endogenous observer (Sec.~\ref{sec:demon}) opens its own program: mapping
the crossover between mode-matched readout, where a fixed sensor reaches $\tS$, and
butterfly-limited readout, where in a ballistic scrambler a fixed sensor dies in $O(1)$
time while only active decoding reaches $\tS$---together with its Landauer budget. That
crossover asks, quantitatively, which dynamics permit memory that simply \emph{sits there}
and stays readable; it is the subject of companion work.
...
The demon sensor of Sec.~\ref{sec:demon} is the boundary contrast
$w=\bar V_0(0)-\bar V_1(0)$ with threshold
$\theta=\tfrac12\bigl(\bar V_0(0)+\bar V_1(0)\bigr)\cdot w$, both computed on the training
half at $t=0$ only; $I\bigl(o(t);F\bigr)$ is the mutual information of the held-out
$2\times2$ confusion of the binary reading $o(t)$. The equilibrium control pre-thermalizes
each world for $10^3$ steps before the demon begins reading, and its verdict statistic is
the run-mean $I(o;F)$ together with the fraction of samples crossing $\tfrac12$ bit (a
run-maximum is positively biased for a noisy plug-in estimator and is not used). The
accumulate-demon recall curve is
$\max_{t_j\ge t_{\mathrm{start}}} I\bigl(o(t_j);F\bigr)$ on the seed-averaged reading
series; ``noise floor'' is its late-time plateau, $0.08$--$0.18$ bit at held-out sample
size $K=40$.
