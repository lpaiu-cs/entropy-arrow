...
experiments/
  ...
  t9c_butterfly.py         T9c: the butterfly limit as a SCALING LAW — in the Clifford brickwork
                           the fixed sensor's horizon knows only the local gate rate (t* ∝ 1/p,
                           size-blind) while t_S ∝ N/p, so κ_demon ∝ 1/N at every p: slowing the
                           gates rescues nothing; the mismatch is structural, not kinetic
...
for e in selftest t1_boundary t2_loschmidt t3_records t3_hard_readout \
         ...
         t9_maxwell_demon t9_demon_universal t9c_butterfly t9d_knudsen; do
...
t_S (T9b). In the scrambler that limit is a **scaling law**: the fixed sensor's horizon knows
only the local gate rate (t\* ∝ 1/p, size-blind) while t_S ∝ N/p, so κ_demon ∝ 1/N at every
rate — slowing the gates rescues nothing; the failure is structural, not kinetic (T9c). And the
transition is **continuous**: sweeping a single collisionality knob (the
